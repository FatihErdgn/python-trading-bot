...
# from adxLinReg import adxLinRegCandles
import WTADX as wdx
import smtplib
import json
...
while True:
    try:
        ...
        raw_data = trading_bot.get_candlestick_data(symbol,timeframe, 250) # raw_data_append = pd.concat([raw_data_append, raw_data]).drop_duplicates().reset_index(drop=True)
        ...
        if strateji == "1":
            ...
            linReg = HumbleLinRegCandles(raw_data)
            linRegSignal = linReg.process()
            
            adx_indicator = wdx.AverageDirectionalIndex(raw_data)
            adx = adx_indicator.calculate()
            
            # adxLinReg = adxLinRegCandles(adx)
            # adxLinRegSignal = adxLinReg.process()            
            
            wt = wdx.WaveTrend(raw_data)
            result_wt = wt.compute()
            
            hullSuite = HullSuiteEmaCross(raw_data)
            rsiDf = hullSuite.calculate_rsi(14)
            rsi = rsiDf['rsi'].astype(float)
            ...
            if ((result_wt['cross_point'].iloc[-3:-1] == 'Long').any() and result_wt['wt2'].iloc[-2] <= 0 and adx['adx'].iloc[-2] >= 35): #or inPositionShort == True or inPositionLong == True
                if linRegSignal['Buy_Signal'].iloc[-2] == True and linRegSignal['Buy_Signal'].iloc[-3] == False:
                    T_F['Buy_Signal'].iloc[0] = True
                    T_F['Sell_Signal'].iloc[0] = False
                
            elif (adx['adx'].iloc[-2] >= 25 and adx['adx'].iloc[-2] <= 35) and ((result_wt['cross_point'].iloc[-3:-1] == 'Long').any() and result_wt['wt2'].iloc[-2] <= -30):
                if linRegSignal['Buy_Signal'].iloc[-2] == True and linRegSignal['Buy_Signal'].iloc[-3] == False:
                    T_F['Buy_Signal'].iloc[0] = True
                    T_F['Sell_Signal'].iloc[0] = False
                
            elif ((result_wt['cross_point'].iloc[-3:-1] == 'Short').any() and result_wt['wt2'].iloc[-2] >= 0 and adx['adx'].iloc[-2] >= 35):
                if linRegSignal['Sell_Signal'].iloc[-2] == True and linRegSignal['Sell_Signal'].iloc[-3] == False:
                    T_F['Buy_Signal'].iloc[0] = False
                    T_F['Sell_Signal'].iloc[0] = True
                    
            elif (adx['adx'].iloc[-2] >= 25 and adx['adx'].iloc[-2] <= 35) and ((result_wt['cross_point'].iloc[-3:-1] == 'Short').any() and result_wt['wt2'].iloc[-2] >= 30):
                if linRegSignal['Buy_Signal'].iloc[-2] == True and linRegSignal['Buy_Signal'].iloc[-3] == False:
                    T_F['Buy_Signal'].iloc[0] = False
                    T_F['Sell_Signal'].iloc[0] = True
            ...
            if inPositionLong == True:
                if (adx['adx'].iloc[-2] >= 25 and adx['adx'].iloc[-2] <= 35) and ((result_wt['cross_point'].iloc[-3:-1] == 'Short').any() and result_wt['wt2'].iloc[-2] >= 20):
                    T_F['Buy_Signal'].iloc[0] = False
                    T_F['Sell_Signal'].iloc[0] = True
                ...
                elif not active_pos.empty and (float(raw_data['close'].iloc[-2])) <= 0.995*(float(active_pos[active_pos.iloc[:, 0] == symbol]['Entry Price'].values[0])):
                      T_F['Buy_Signal'].iloc[0] = False
                      T_F['Sell_Signal'].iloc[0] = True
            ...
            elif inPositionShort == True:
                if (adx['adx'].iloc[-2] >= 25 and adx['adx'].iloc[-2] <= 35) and ((result_wt['cross_point'].iloc[-3:-1] == 'Long').any() and result_wt['wt2'].iloc[-2] <= -20):
                    T_F['Buy_Signal'].iloc[0] = True
                    T_F['Sell_Signal'].iloc[0] = False
                    
                elif ((result_wt['cross_point'].iloc[-3:-1] == 'Long').any() and result_wt['wt2'].iloc[-2] <= 0 and adx['adx'].iloc[-2] >= 35):
                    T_F['Buy_Signal'].iloc[0] = True
                    T_F['Sell_Signal'].iloc[0] = False
                    
                elif not active_pos.empty and 1*(float(raw_data['close'].iloc[-2])) >= 1.005*(float(active_pos[active_pos.iloc[:, 0] == symbol]['Entry Price'].values[0])):
                      T_F['Buy_Signal'].iloc[0] = True
                      T_F['Sell_Signal'].iloc[0] = False
                ...
                if (inPositionLong == False and inPositionShort == False) and (T_F['Sell_Signal'].iloc[0] == True or T_F['Sell_Signal'].iloc[0] == True):
                    if (result_wt['cross_point'].iloc[-3:-1] == 'Long').any() and (result_wt['cross_point'].iloc[-3:-1] == 'Short').any():
                        T_F['Buy_Signal'].iloc[0] = False
                        T_F['Sell_Signal'].iloc[0] = False
