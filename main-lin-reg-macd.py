...
from OBVMACD import OBVMACDIndicator
import smtplib
import json
...
while True:
    try:
        ...
        raw_data = trading_bot.get_candlestick_data(symbol,timeframe, 250)
        ...
        if strateji == "1":
            ...
            obvMacd = OBVMACDIndicator(raw_data)
            obvMacdSignal = obvMacd.get_dataframe()
            ...
            if ((rsi.iloc[-14:] >= 65).any() or (rsi.iloc[-14:] <= 35).any()) or inPositionShort == True or inPositionLong == True:
                if linRegSignal['Buy_Signal'].iloc[-2] == True and linRegSignal['Buy_Signal'].iloc[-3] == False and obvMacdSignal['Buy_Signal'].iloc[-1] == True:
                    T_F['Buy_Signal'].iloc[0] = True
                    T_F['Sell_Signal'].iloc[0] = False
                elif linRegSignal['Sell_Signal'].iloc[-2] == True and linRegSignal['Sell_Signal'].iloc[-3] == False and obvMacdSignal['Sell_Signal'].iloc[-1] == True:
